...
public class BundleRepositoryImpl implements BundleRepositoryCustom{
    private final JdbcTemplate jdbcTemplate;


    @Override
    public List<UUID> findIdsWithUpdatedChanged(List<BundleDTO> dtos) {
        ...
        String sql = """
            ...
            missing AS (
                SELECT r.id
                FROM remote_data r
                LEFT JOIN bundles p ON p.id = r.id
                WHERE p.id IS NULL
            )
